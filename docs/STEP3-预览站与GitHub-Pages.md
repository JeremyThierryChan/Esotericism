...
配色改为拜占庭艺术的三主色体系（`apps/preview/src/styles.css`）：

| 主色 | 角色 | 依据 |
|---|---|---|
| **拜占庭紫 Porphyry** | **底色**（斑岩） | 拜占庭皇权之色；斑岩（porphyry）是带紫调的红褐石 |
| **圣像金 Icon Gold** | **强调与状态** | 圣像画的金箔背光 |
| **石膏灰白 Gesso** | **文字与浅色底** | 圣像画板底层石膏底 |

**关联色**（拜占庭马赛克与手抄本常见材质，用于区分语义，避免全站只有紫金两色）：
青金石 Lapis（历史/迁移类标签）、孔雀石 Malachite（规则判分、命中）、
朱砂 Cinnabar（错误、未命中、违规）、玫瑰茜 Rose Madder（AI / Rubric 判分类标签）。

### 两套主题

| 主题 | 触发 | 方案 |
|---|---|---|
| **夜紫宫廷**（默认·深色） | 默认 | 斑岩底 + 金强调 + 石膏白字 |
| **石膏画板**（浅色） | `prefers-color-scheme: light` | 石膏底 + 金强调 + 斑岩字 |

> 「石膏灰白」只有在浅色主题里才真正成为**底**（与圣像画板的实际做法一致），
> 所以两套都做了：深色用石膏作字、浅色用石膏作底，三主色在两套里都出现。

### ⚠️ 两处刻意的诚实标注

1. **颜色名称取自材质，但十六进制值是设计选择，不是历史色度测量值。**
   历史上的「帝王紫」是骨螺紫（Tyrian purple，约 `#66023C` 一系），此处为屏幕可读性做了调整，
   **不宣称还原古物原色**。这条标注写在 `styles.css` 顶部注释里 —— 本项目的来源纪律同样适用于设计。
2. **`--accent-bright` 已改名为 `--accent-strong`。** 原因是一个真实的 bug：
   「更亮的金」在深色主题里是强调，在浅色主题里却是**看不清**。名字不该编码方向。
   改名后两套主题各自映射（深色更亮 / 浅色更深），横幅粗体另设 `--banner-strong`。

### 对比度是被脚本验证的，不是靠眼睛猜

新增 `apps/preview/scripts/contrast.mjs`，**已接入构建门**（`build` 第一步，不过就不产出产物）：

```
pnpm --filter @dlg/preview contrast
```

它解析 `styles.css` 的 CSS 变量、递归解析 `var()` 引用，按 **WCAG 2.1** 计算：
- **30 组配对**（正文 / 次级 / 最弱文字 / 金 / 四种标签 / 四种判分徽标 / 选项卡 / 横幅 / 结构边框）
- **两套主题都算**，全部要求 ≥ 4.5:1（页面有大量 11.5–13px 小字，故不放松到 3:1）
- 附带检查**所有 `var()` 引用都已定义**（改名最容易留下断链）

实测：深色最低 4.73、浅色最低 5.11，**30/30 全达标**。
它挡住的真实问题：浅色主题原本有 3 项不达标（`ink-faint` 3.35、金按钮悬停 3.58、横幅粗体 3.16）。
