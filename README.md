...
内部预览站（GitHub Pages，纯静态）。可以看到：内容库状态、**练习区**（25 道规则题自动生成、
可提交、实时判分、实时积分与掌握度）、一道开放题（Rubric 自查）、跨体系对应关系的文献链、
以及每条内容的来源强度与审核状态。
...
step 4  自己当用户玩两周（纯规则闭环已就绪）            🔶 闭环 ✓ / 攒数据 ⬜
step 5  学习者端 UI                                  ⬜
step 6  内容规模化                                   ⬜
────── 设计文档阶段（step 5 之后才需要）────────────────────────────
V0.3  跨体系知识图谱                                ⬜
...
| **T3** | **R1 证据清单复核** | 🔶 **部分完成**：R1 Q2.2（五行生克）已由一手古典文献取代/加强；**R1 Q1.x（塔罗↔占星）与 Q3.2（《增删卜易》成书年份）本轮未加强** —— 未取得 *Book T* 原条目、《春秋繁露·五行相胜》原文、成书年份。 | `docs/复核记录-T1-T3.md` §五 |
